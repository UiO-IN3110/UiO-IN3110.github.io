...
def final():
    ...
    # Record some statistics
    game_states[game_id]["changed_choice"] = (
        selected != game_states[game_id]["first_choice"]
    )
...
def statistics():
    changed_and_won = [
        e["won"]
        for e in game_states.values()
        if "changed_choice" in e and e["changed_choice"]
    ]
    notchanged_and_won = [
        e["won"]
        for e in game_states.values()
        if "changed_choice" in e and not e["changed_choice"]
    ]

    s1 = "Changed and won: {} out of {}".format(
        sum(changed_and_won), len(changed_and_won)
    )
